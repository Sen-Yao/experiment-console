...
[[ "$BACKUP_DIR" =~ ^/[A-Za-z0-9._/-]+$ && "$BACKUP_DIR" == "$REMOTE_BASE/backups/"* ]] || {
  echo "Backup must be under $REMOTE_BASE/backups" >&2
  exit 2
}
echo "route=$SSH_USER@$SSH_HOST:$SSH_PORT"
echo "backup=$BACKUP_DIR"
if [[ "$APPLY" != "1" ]]; then
  echo "DRY RUN: no SSH, service stop, or data restore was performed."
  exit 0
fi
ssh -p "$SSH_PORT" -o BatchMode=yes -o ConnectTimeout=15 "$SSH_USER@$SSH_HOST" /bin/bash \
  "$REMOTE_BASE/current/deploy/yggdrasil/rollback-release.sh" \
  --base "$REMOTE_BASE" --backup-dir "$BACKUP_DIR" --apply
